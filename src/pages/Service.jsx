...
const Service = () => {
  const { id } = useParams();
  const filteredItem = data.filter((item) => item.id === parseInt(id));
  const { image, title, desc } = filteredItem[0];
  return (
    <div className='container mx-auto my-16 h-[500px] text-center border border-gray-200 rounded-2xl bg-transparent text-gray-200 p-12 flex flex-col justify-around'>
